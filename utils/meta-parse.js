...
export const UAParse = e => {
  ...
  if (r = e.match(/MSIE\s([^\s|]+)/gi)) {
    ...
    outputer = '<span class="ua_2345explorer"><i class="iconfont icon-wangluo-"></i>2345王牌浏览器'
  } else if (r = e.match(/2345chrome/ig)) {
    outputer = '<span class="ua_2345chrome"><i class="iconfont icon-wangluo-"></i>2345加速浏览器'
  } else if (r = e.match(/LBBROWSER/ig)) {
    outputer = '<span class="ua_lbbrowser"><i class="iconfont icon-liebao"></i>猎豹安全浏览器'
  } else if (r = e.match(/MicroMessenger\/([^\s]+)/ig)) {
    const r1 = r[0].split('/');
    outputer = `<span class="ua_qq"><i class="iconfont icon-wechat-fill"></i>微信 | ${r1[1].split('/')[0]}`
  } else if (r = e.match(/QQBrowser\/([^\s]+)/ig)) {
    const r1 = r[0].split('/');
    outputer = `<span class="ua_qq"><i class="iconfont icon-qqie"></i>QQ浏览器 | ${r1[1].split('/')[0]}`
  } else if (r = e.match(/QQ\/([^\s]+)/ig)) {
    const r1 = r[0].split('/');
    outputer = `<span class="ua_qq"><i class="iconfont icon-qqie"></i>QQ浏览器 | ${r1[1].split('/')[0]}`
  } else if (r = e.match(/MiuiBrowser\/([^\s]+)/ig)) {
    const r1 = r[0].split('/');
    outputer = `<span class="ua_mi"><i class="iconfont icon-tubiaozhizuomoban"></i>Miui浏览器 | ${r1[1].split('/')[0]}`
  } else if (r = e.match(/Chrome([\d]*)\/([^\s]+)/ig)) {
    const r1 = r[0].split('/');
    outputer = `<span class="ua_chrome"><i class="iconfont icon-chrome-fill"></i>Chrome | ${r1[1].split('.')[0]}`
    ...
    outputer = '<span class="ua_other"><i class="iconfont icon-wangluo-"></i>其它浏览器'
  }
...
export const OSParse = e => {
  ...
  if (e.match(/win/ig)) {
    ...
    os = '<span class="os_linux"><i class="iconfont icon-unix"></i>Linux'
  } else if (e.match(/iphone/ig)) {
    os = '<span class="os_mac"><i class="iconfont icon-apple-fill"></i>iPhone OS'
  } else if (e.match(/mac/ig)) {
    os = '<span class="os_mac"><i class="iconfont icon-apple-fill"></i>Mac OS X'
  } else if (e.match(/unix/ig)) {
    os = '<span class="os_unix"><i class="iconfont icon-unix"></i>Unix'
  } else {
    os = '<span class="os_other"><i class="iconfont icon-iphone"></i>Other'
  }
  return `${os}</span>`
};
